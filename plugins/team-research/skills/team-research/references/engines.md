...
| `scout` | Phase 1 | claude, codex, kimi, grok, cursor | one-shot, read-only landscape scan |
...
| `critic` | Phase 3, on demand | claude, codex, kimi, grok, cursor | failure-mode analysis of the flagged areas |
| `specialist` | Phase 3, on demand | claude, codex, kimi, grok, cursor | deep dive into one flagged area |
...
2. **Run the CLI** with the preset below, foreground, `timeout: 600000`, output redirected to
   `.claude/teams/research-<topic-slug>/engine/<role>-<n>.out.txt`.
...
mode — every external role in this plugin reads. `{prompt}` is always `"$(cat <prompt file>)"`.
...
Kimi has no sandbox flag: its read-only boundary is the instruction alone. Acceptable for these
read-only roles; say so once in the 📢 line.
